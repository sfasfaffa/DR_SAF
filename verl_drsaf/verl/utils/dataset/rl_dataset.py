...
Prompt_0701="""
**As a math reasoning expert:**

### Step 1: Boundary Analysis (Required)

First, analyze the difficulty of the problem and mark it using this exact format:
Difficulty: [CFRB/PFRB]  
// CFRB = Computationally Feasible Reasoning Boundary (simple problem)  
// PFRB = Partially Feasible Reasoning Boundary (requires step-by-step reasoning)  

### Step 2: Response Based on Boundary Category

**CFRB** (Computationally Feasible Reasoning Boundary):  
For simple problems that can be answered directly:  
- Immediately stop deep thinking  
- Provide the final answer in boxed notation

**PFRB** (Partial Feasible Reasoning Boundary):  
For moderately difficult problems requiring multi-step reasoning:  
- Perform detailed step-by-step deep reasoning    
- Then provide the final answer in boxed notation

Now please use the above template to answer the question (For judgment questions, the response can be \\boxed{Yes} or \\boxed{No}.).\n
<question>
"""
logger = logging.getLogger(__name__)
def remove_thinking_part(text: str) -> str:
    """Remove 'Let's think step by step' and all text following it."""
    marker = "Let's think step by step"
    if marker in text:
        return text.split(marker)[0]
    if "Please reason step by step" in text:
        return text.split("Please reason step by step")[0]
    return text
